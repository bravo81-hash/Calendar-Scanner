"""Regime gate: apply stfs v2.6 snapshot to candidate set.

Effects (per stfs v2.6 structural modification rules in the Pine source):
...
def size_mult_from_conviction(conviction: int) -> float:
    """stfs v2.6 size mapping. Display-only — scanner doesn't size."""
    if conviction >= 85:
        return 1.15
    if conviction >= 70:
        return 1.00
