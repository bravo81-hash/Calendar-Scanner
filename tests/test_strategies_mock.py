...
class StrategyMockTests(unittest.TestCase):
    ...
    def test_regime_uses_stfs_27_a14_and_flyagonal_scores(self):
        candidates = [
            CalendarCandidate(
                strategy="a14_bwb", symbol="SPX", legs=[],
                front_expiry="20260520", back_expiry="20260520",
                front_dte=7, back_dte=7,
            ),
            CalendarCandidate(
                strategy="fly_diagonal", symbol="SPX", legs=[],
                front_expiry="20260520", back_expiry="20260527",
                front_dte=7, back_dte=14,
            ),
        ]
        regime = RegimeSnapshot(stfs_score_a14=100, stfs_score_flyagonal=0)

        apply_regime(candidates, regime)

        self.assertAlmostEqual(candidates[0].regime_score, 1.5, places=2)
        self.assertAlmostEqual(candidates[1].regime_score, 0.5, places=2)
