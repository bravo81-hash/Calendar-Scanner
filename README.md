...
Additional reference strategies are also preserved but not yet implemented as live scanner builders:

- `A14.html` — SPX put broken-wing butterfly with optional calendar hedges.
- `HV7.html` — high-volatility-event put broken-wing butterfly.
- `FlyDiagonal.html` — call BWB plus put diagonal / time-spread family.

---

## Regime gate (stfs v2.6 integration)

Your TradingView `stfs v2.6` Pine indicator's outputs feed the scanner via a manual sidebar paste. Effects:
...
3. Multi-leg multi-expiry candidates (Triple = 6 legs across 2 expiries; TimeZone = 4 legs hybrid PCS+cal)
...
- **regime_score**: stfs v2.6 multiplier (1.0 = neutral, 0 = hard skip)
...
│   ├── REGIME_INTEGRATION.md    # stfs v2.6 ingestion contract
│   ├── strategies_html/         # Strategy HTML references + Pine
│   └── reference/               # Buddy's original app.py (non-runnable here)
...
│   ├── regime.py                # stfs v2.6 snapshot → candidate multipliers
...
- Original SPX Live Calendar Scanner v1.3-1.5 by Bhavik's trading buddy → preserved at `docs/reference/buddy_app.py`
- stfs v2.6 Pine indicator → preserved at `docs/strategies_html/stfs v2.6.pine`
