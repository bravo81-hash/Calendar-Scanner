# Regime Integration — STFS v2.7 → Calendar Scanner

The TradingView Pine script `STFS v2.7` (preserved at `docs/strategies_html/stfs v2.7.pine`) computes a regime context state, macro regime, per-strategy scores, and structural-modification flags. The scanner ingests a snapshot of these values once per session (or whenever you re-check the dashboard) and applies them to candidate rankings.

## Ingestion contract (v1)

Manual paste via the sidebar **Regime snapshot (STFS v2.7)** expander. Fields:

| Sidebar field | STFS v2.7 source | Effect |
|---------------|------------------|--------|
...
| A14 score | `score_a14` from STFS | 0-100 → 0.5×-1.5× linear multiplier |
| Flyagonal score | `score_fly` from STFS | 0-100 → 0.5×-1.5× linear multiplier |
...
| Triple Calendar score | no direct STFS score | 0-100 → 0.5×-1.5× linear multiplier; leave at 50 unless you map it manually |
...
## STFS v2.7 features we use

From the Pine source (`docs/strategies_html/stfs v2.7.pine`):
...
- **`score_a14`, `score_fly`, `score_te`, `score_tz`, `score_dir`**: per-strategy continuous weighted scores.
